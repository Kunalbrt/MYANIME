...
// â”€â”€ API Base URL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// âœ… FIX: removed leading space and added /api suffix
const API ='https://myanime-backend-2vc9.onrender.com/api';

// â”€â”€ Auth Helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const DEMO_ANIME = [
  ...
  { id:'d10',title:'Hunter x Hunter',type:'series',genre:'adventure',year:'2011',rating:'TV-14',desc:'Gon Freecss aspires to become a Hunter, an exceptional being capable of greatness.',thumb:'',videoUrl:'',emoji:'ðŸŽ¯',trending:true,topRated:false },
  { id:'d11',title:'Jujutsu Kaisen',type:'series',genre:'action',year:'2020',rating:'TV-MA',desc:'A boy swallows a cursed talisman and becomes host to a powerful demon. Now he must battle demons and allies alike to avoid being killed.',thumb:'',videoUrl:'',emoji:'ðŸ‘ï¸',trending:true,topRated:true },
  { id:'d12',title:'Sword Art Online',type:'series',genre:'fantasy',year:'2012',rating:'TV-14',desc:'In the near future, a virtual reality massive multiplayer online role-playing game called Sword Art Online is released.',thumb:'',videoUrl:'',emoji:'ðŸ—¡ï¸',trending:false,topRated:false },
];
...
// â”€â”€ Admin Auth State â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ 2FA OTP State â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let otpCode = '';
let otpExpiry = 0;
let otpAttempts = 0;
...
// â”€â”€ Analytics State â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let analytics = {
...
function init() {
  ...
  trackVisit();
  updateNavForAuth();
  fetchAnimeFromBackend(); // fetch last so local data shows first
...
async function addAnime() {
  ...
  // Step 1 â€” Upload thumbnail to Cloudinary if a local image was selected
  let thumbnailUrl = '';
  if (uploadedThumbBlob && uploadedThumbBlob.startsWith('data:')) {
    try {
      showToast('â³ Uploading thumbnail...');
      ...
      if (thumbData.success) {
        ...
        showToast('âš ï¸ Thumbnail upload failed: ' + thumbData.message);
      }
    } catch (err) {
      console.log('Thumbnail upload error:', err.message);
      showToast('âš ï¸ Thumbnail upload failed, saving without it.');
    }
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      saveToStorage();
      renderAll();
      showToast(`âœ… "${title}" saved to database!`);
      // Reset form
...
function refreshAnalytics() {
  ...
  if (deviceList) {
    ...
    const icons = { Desktop:'ðŸ–¥ï¸', Mobile:'ðŸ“±', Tablet:'ðŸ“Ÿ' };
    deviceList.innerHTML = Object.keys(analytics.devices).length === 0
      ...
      : Object.entries(analytics.devices).map(([name, cnt]) => `
          <div class="top-list-item">
            <div class="top-list-rank">${icons[name]||'ðŸ’»'}</div>
            <div class="top-list-bar-wrap">
  ...
  if (browserList) {
    ...
    browserList.innerHTML = Object.keys(analytics.browsers).length === 0
      ...
      : Object.entries(analytics.browsers).map(([name, cnt]) => `
          <div class="top-list-item">
            <div class="top-list-rank">${icons[name]||'ðŸŒ'}</div>
            <div class="top-list-bar-wrap">
              <div class="top-list-name">${name}</div>
  ...
  showToast('ðŸ“Š Analytics refreshed');
}
...
function openAdminLogin() {
  ...
  if (Date.now() < lockoutUntil) {
    ...
    info.textContent = `ðŸ”’ Too many failed attempts. Try again in ${mins} minute(s).`;
  }
...
function checkAdminPassword() {
  if (Date.now() < lockoutUntil) {
    const mins = Math.ceil((lockoutUntil - Date.now()) / 60000);
    showToast(`ðŸ”’ Locked out for ${mins} more minute(s).`);
    return;
  }
  ...
  if (input === ADMIN_PASSWORD) {
    ...
    if (loginAttempts >= MAX_ATTEMPTS) {
      ...
      const info = document.getElementById('loginAttemptInfo');
      info.style.display = 'block';
      info.textContent = 'ðŸ”’ Too many failed attempts! Locked for 30 minutes.';
      document.getElementById('adminPasswordInput').value = '';
    } else {
      saveSecurity();
...
function grantAdminAccess() {
  ...
  showPage('admin');
  addLoginEntry('success', 'âœ… Login successful');
...
function onPasswordTyped() {
  ...
  if (val.length > 0 && !otpPrepared) {
    ...
    showToast('âš ï¸ Login attempt detected â€” OTP sent to your email!');
  }
}
...
function _sendOTPEmail(code) {
  if (typeof emailjs !== 'undefined' && emailJSConfig.pubkey && emailJSConfig.service && emailJSConfig.template) {
    ...
    emailjs.send(emailJSConfig.service, emailJSConfig.template, {
      ...
      showToast('ðŸ“§ OTP sent to ' + ADMIN_EMAIL);
    }).catch(err => {
      console.error('EmailJS error:', err);
      showToast('âš ï¸ Could not send OTP email. Check EmailJS config.');
    });
  } else {
    console.log('%cðŸ” DEV MODE OTP: ' + code, 'color:#e50914;font-size:20px;font-weight:bold;');
    showToast('ðŸ“§ OTP: ' + code + ' (EmailJS not configured)');
  }
}
...
function initiate2FA() {
  if (!otpPrepared || !otpCode) {
    ...
    showToast('ðŸ“§ OTP already sent to ' + ADMIN_EMAIL);
  }
  otpPrepared = false;
  ...
  resendBtn.textContent = 'ðŸ”„ Resend Code (wait 30s...)';
...
function startOTPTimer() {
  ...
  otpTimerInterval = setInterval(() => {
    ...
    if (left <= 0) {
      ...
      err.style.display = 'block'; err.textContent = 'â±ï¸ Code expired. Please try again.';
...
function verifyOTP() {
  ...
  if (!entered) { errEl.style.display = 'block'; errEl.textContent = 'âš ï¸ Please enter your OTP code.'; return; }
  if (Date.now() > otpExpiry) { errEl.style.display = 'block'; errEl.textContent = 'â±ï¸ Code expired. Please resend.'; return; }
  otpAttempts++;
  if (entered === otpCode) {
    grantAdminAccess();
  } else {
...
function toggle2FA() {
  ...
  saveSecurity();
  showToast(twoFAEnabled ? 'âœ… 2FA enabled' : 'âš ï¸ 2FA disabled');
}
...
function testEmailJS() {
  ...
  if (!pk || !sv || !tm) { showToast('âš ï¸ Fill in all EmailJS fields first.'); return; }
  if (typeof emailjs === 'undefined') { showToast('âš ï¸ EmailJS library not loaded.'); return; }
  emailjs.init(pk);
...
function lockAdminNow() {
  ...
  showPage('home');
  showToast('ðŸ”’ Admin locked.');
}

function resetAllData() {
  if (!confirm('âš ï¸ This will reset ALL site data including library, settings, and analytics. Are you absolutely sure?')) return;
  if (!confirm('Final confirmation: Delete everything?')) return;
  localStorage.clear();
  showToast('ðŸ’¥ All data reset. Reloading...');
...
function renderHero() {
  ...
  // âœ… FIX: support Cloudinary https:// URLs not just data: blobs
...
function applyRowSettings() {
  ...
  showToast('âœ… Home page updated!');
}
...
// âœ… FIX: helper so all render functions check http:// and https:// too
...
function renderMyList() {
  ...
  const edits = getSiteEdits();
  if (myList.length === 0) {
    grid.innerHTML = `<div class="empty-list"><div style="font-size:4rem">ðŸ“‹</div><p>${edits.emptyList || 'Your list is empty. Add anime to watch later!'}</p></div>`;
    return;
  }
...
function openPlayer(anime, episodeUrl, episodeTitle) {
  ...
  document.getElementById('playerInfoMeta').textContent  = ` â€¢ ${anime.year||''} â€¢ ${anime.genre||''} â€¢ ${anime.rating||''}`;
...
function setResolution(res) {
  ...
  document.getElementById('resBtn').textContent = `âš™ï¸ ${res === 'auto' ? 'Auto' : res}`;
  ...
  showToast(res === 'auto' ? 'âœ… Resolution: Auto' : `âš™ï¸ Resolution set to ${res}`);
}
...
function simulateUpload(file) {
  ...
  reader.onload = e => {
    ...
    fill.style.width = '100%'; percent.textContent = '100% âœ“';
    uploadedVideoBlob = e.target.result;
    showToast('âœ… Video loaded! Fill in details and click Add to Library.');
  };
  reader.readAsDataURL(file);
}
  ...
  animeLibrary.forEach(anime => {
    ...
    item.innerHTML = `
      ...
      <div class="library-info">
        ...
        <span>${anime.type} â€¢ ${anime.year||'N/A'} â€¢ ${anime.genre||''}</span>
        <span style="color:${anime.videoUrl?'#4caf50':'#e57373'};font-size:0.7rem">
          ${anime.videoUrl ? 'âœ… Has video source' : 'âš ï¸ No video source'}
        </span>
      </div>
...
async function saveAnimeEdit() {
  ...
  showToast(`âœ… "${anime.title}" updated!`);
}
...
function testVideoUrl() {
  ...
  if (!url) { showToast('âš ï¸ Enter a URL to test'); return; }
...
function convertGDriveUrl() {
  ...
  if (!match) { showToast('âš ï¸ Invalid Google Drive URL format.'); return; }
  const fileId    = match[1];
  const directUrl = `https://drive.google.com/uc?export=download&id=${fileId}`;
  result.style.display = 'block';
  result.innerHTML = `
    <p>âœ… Direct stream URL:</p>
    <code>${directUrl}</code>
    <p style="margin-top:0.5rem;color:#e57373;font-size:0.78rem">âš ï¸ Note: Google Drive may block large video streaming due to CORS. Cloudinary or S3 work better.</p>
...
async function handleLogin() {
  ...
  try {
    ...
    if (data.success) {
      ...
      showToast(`âœ… Welcome back, ${data.user.username}!`);
...
async function handleSignup() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) { closeUserAuth(); showToast('âœ… Account created! Check your email to verify.'); }
    else { errEl.textContent = data.message; errEl.style.display = 'block'; }
  } catch(e) { errEl.textContent = 'Connection error. Is server running?'; errEl.style.display = 'block'; }
}
